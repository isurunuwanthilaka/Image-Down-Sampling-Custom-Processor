...
for line in range(len(file)):
	...
	if any(words[0][i:]=='NOP' for i in range(0,len(words[0]),4)):
		...
		memory_count+=1
		continue
	elif any(words[0][i:]=='DECA' for i in range(0,len(words[0]),4)):
		file1.append("1101"+address[words[1]]+"000,")
		memory_count+=1
		continue
...
for i in range(len(file1)):
	if file1[i][:4]=='step':
		file1[i]=('{0:08b}'.format(step[file1[i]])+",")
file=file1
